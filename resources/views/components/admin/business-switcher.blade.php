{{--
    ...
    Shown to anyone who has somewhere to switch *to* — a Super Admin (every
    active business) or an admin who has been assigned extra businesses. It used
    to be gated on the Super Admin role alone, which meant the only way to let
    an external accountant see a second company's books was to make them a
    Super Admin.

    The list is always the set the viewer can actually reach, so nothing on
    screen 403s when clicked.
--}}
@php
    ...
    $switchable = $admin?->canSwitchBusiness() ?? false;

    $options = $switchable
        ? ($isSuper
            ? \App\Models\Business::where('is_active', true)->orderBy('name')->get()
            : $admin->accessibleBusinesses())
        : collect();
@endphp

@if($admin && $switchable && $options->isNotEmpty())
