<x-layout.admin title="Edit Category">
    <div x-data="{ name: '{{ old('name', $category->name) }}', slug: '{{ old('slug', $category->slug) }}', isActive: {{ old('is_active', $category->is_active) ? 'true' : 'false' }} }">
        <x-admin.breadcrumb :items="[['label'=>'Categories','url'=>route('admin.categories.index')],['label'=>'Edit Category']]" />
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <x-admin.searchable-select name="parent_id" :options="$parentCategories" :selected="$category->parent_id" placeholder="-- None --" />
                    </div>
                    <div>
                        <label for="sort_order">Sort Order</label>
                        <input id="sort_order" name="sort_order" type="number" min="0" class="form-input" value="{{ old('sort_order', $category->sort_order) }}" />
                    </div>
                    <div class="md:col-span-2">
                        <label for="description">Description</label>
                        <textarea id="description" name="description" class="form-input" rows="3" placeholder="Category description...">{{ old('description', $category->description) }}</textarea>
                    </div>
                    <div>
