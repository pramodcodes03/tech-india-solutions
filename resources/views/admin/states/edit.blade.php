<x-layout.admin title="Edit State">
    <div>
        <x-admin.breadcrumb :items="[['label'=>'States','url'=>route('admin.states.index')],['label'=>'Edit State']]" />

        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Edit State</h5>
            ...
            <form action="{{ route('admin.states.update', $state->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input id="name" name="name" type="text" class="form-input" value="{{ old('name', $state->name) }}" required />
                    </div>
                    <div>
                        <label for="code">Code</label>
                        <input id="code" name="code" type="text" maxlength="10" class="form-input" value="{{ old('code', $state->code) }}" placeholder="e.g. MH" />
                    </div>
                </div>
