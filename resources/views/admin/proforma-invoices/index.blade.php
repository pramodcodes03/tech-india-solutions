<x-layout.admin title="Proforma Invoices">
    <div x-data="proformaList">
        <x-admin.breadcrumb :items="[['label' => 'Proforma Invoices']]" />

        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Proforma Invoices</h5>
            <div class="flex items-center gap-3 flex-wrap">
                <div class="relative">
                    ...
                    <div class="absolute ltr:right-[11px] rtl:left-[11px] top-1/2 -translate-y-1/2 peer-focus:text-primary">
                        <svg class="mx-auto" width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="11.5" cy="11.5" r="9.5" stroke="currentColor" stroke-width="1.5" opacity="0.5"></circle><path d="M18.5 18.5L22 22" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>
                    </div>
                </div>
                ...
                <button type="button" class="btn btn-outline-danger btn-sm" x-show="searchText || filterStatus" @click="clearFilters()">Clear</button>
                <a href="{{ route('admin.proforma-invoices.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
        ...
        <div class="panel px-0 border-[#e0e6ed] dark:border-[#1b2e4b]">
                ...
                <table class="table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2">Proforma #</th>
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2 font-semibold" x-text="item.proforma_number"></td>
                                <td class="px-4 py-2" x-text="item.customer ? item.customer.name : '-'"></td>
                                <td class="px-4 py-2" x-text="formatDate(item.proforma_date)"></td>
                                <td class="px-4 py-2" x-text="formatDate(item.valid_until)"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/proforma-invoices') }}/${item.id}`" class="btn btn-sm btn-outline-info p-1.5" data-tippy-content="View"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        <a :href="`{{ url('admin/proforma-invoices') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary p-1.5" data-tippy-content="Edit" x-show="item.status !== 'converted'"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                                        <a :href="`{{ url('admin/proforma-invoices') }}/${item.id}/pdf`" class="btn btn-sm btn-outline-secondary p-1.5" target="_blank" data-tippy-content="PDF"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg></a>
                                        <button type="button" class="btn btn-sm btn-outline-warning" @click="cloneItem(item.id)">Clone</button>
                                        <button type="button" class="btn btn-sm btn-outline-success" x-show="item.status !== 'converted' && !item.invoice_id" @click="convertToInvoice(item.id)">Convert to Invoice</button>
                                        <a :href="`{{ url('admin/invoices') }}/${item.invoice_id}`" class="btn btn-sm btn-success" x-show="item.invoice_id">Invoice</a>
                                        <button type="button" class="btn btn-sm btn-outline-danger p-1.5" @click="deleteItem(item.id)" x-show="item.status !== 'converted'" data-tippy-content="Delete"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                    </div>
                                </td>
                            </tr>
                        </template>
                            ...
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    <div class="flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 dark:bg-primary/20">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                                    </div>
                                    <p class="text-base font-semibold text-gray-700 dark:text-gray-200">No proforma invoices yet</p>
                                    <a href="{{ route('admin.proforma-invoices.create') }}" class="btn btn-primary btn-sm mt-1 gap-1">+ Create Proforma</a>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3" x-show="pagination.last_page > 1">
                ...
                <div class="flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-outline-primary px-2.5" @click="changePage(pagination.current_page - 1)" :disabled="pagination.current_page === 1">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary px-2.5" @click="changePage(pagination.current_page + 1)" :disabled="pagination.current_page === pagination.last_page">
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('proformaList', () => ({
                items: @json($proformas->items()),
                pagination: {
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.proforma-invoices.index') }}?page=${page}`;
                ...
                statusBadge(status) {
                    const map = { draft: 'bg-dark', sent: 'bg-info', accepted: 'bg-success', rejected: 'bg-danger', expired: 'bg-warning', converted: 'bg-primary' };
                ...
                cloneItem(id) {
                    fetch(`{{ url('admin/proforma-invoices') }}/${id}/clone`, {
                        method: 'POST',
                ...
                convertToInvoice(id) {
                    const swal = window.Swal.mixin({ confirmButtonClass: 'btn btn-success', cancelButtonClass: 'btn btn-outline-secondary ltr:mr-3 rtl:ml-3', buttonsStyling: false });
                    swal.fire({ title: 'Convert to Tax Invoice?', text: 'This will create a new tax invoice from this proforma.', icon: 'question', showCancelButton: true, confirmButtonText: 'Yes, convert!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/proforma-invoices') }}/${id}/convert-to-invoice`, {
                                method: 'POST',
                                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                            })
                            .then(res => res.json())
                ...
                deleteItem(id) {
                    ...
                    swal.fire({ title: 'Are you sure?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/proforma-invoices') }}/${id}`, {
                                method: 'DELETE',
                                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                            })
                            .then(res => res.json())
