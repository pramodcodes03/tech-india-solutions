<x-layout.admin title="Create Proforma Invoice">
    <div x-data="proformaForm()">
        <x-admin.breadcrumb :items="[['label'=>'Proforma Invoices','url'=>route('admin.proforma-invoices.index')],['label'=>'Create Proforma']]" />

        <div class="flex items-center justify-between mb-6">
            <div>
                <h5 class="text-xl font-bold dark:text-white-light">Create Proforma Invoice</h5>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Non-binding invoice issued before the actual sale</p>
            </div>
            <a href="{{ route('admin.proforma-invoices.index') }}" class="btn btn-outline-primary gap-2">
        ...
        <form action="{{ route('admin.proforma-invoices.store') }}" method="POST">
            @csrf
            ...
            <div class="panel mb-5 !p-0">
                <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-primary/15 rounded-t-xl">
                    <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/20">
                ...
                <div class="p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Customer <span class="text-danger">*</span></label>
                            <x-admin.searchable-select name="customer_id" :options="$customers" placeholder="-- Select Customer --" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Proforma Date <span class="text-danger">*</span></label>
                            <input name="proforma_date" type="date" class="form-input" value="{{ old('proforma_date', date('Y-m-d')) }}" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Valid Until</label>
                            <input name="valid_until" type="date" class="form-input" value="{{ old('valid_until', date('Y-m-d', strtotime('+15 days'))) }}" />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Status <span class="text-danger">*</span></label>
            ...
            <div class="panel mb-5 !p-0">
                <div class="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-info/10 via-info/5 to-transparent border-b border-info/15">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-info/20">
                    ...
                    <button type="button" class="btn btn-info btn-sm gap-2" @click="addItem()">+ Add Row</button>
                </div>
                        ...
                        <div class="px-5 py-4">
                            <div class="flex gap-3 items-start mb-3">
                                ...
                                <div class="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    ...
                                    <div>
                                        <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">HSN / SAC Code</label>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                <div class="panel !p-0">
                    <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-warning/10 via-warning/5 to-transparent border-b border-warning/15">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-warning/20">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            ...
                            <textarea name="terms" class="form-input resize-none" rows="5" placeholder="Advance payment terms, delivery conditions...">{{ old('terms', 'This is a Proforma Invoice and not a Tax Invoice. Advance payment as per agreed terms.') }}</textarea>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Internal Notes</label>
                            <textarea name="notes" class="form-input resize-none" rows="3" placeholder="Internal remarks...">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel !p-0 flex flex-col">
                    <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-success/10 via-success/5 to-transparent border-b border-success/15">
                        <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-success/20">
                    ...
                    <div class="p-5 flex-1 space-y-4">
                        ...
                        <div class="rounded-lg border border-gray-200 dark:border-[#1b2e4b] overflow-hidden">
                            ...
                            <div class="flex gap-3 p-3">
                                ...
                                <input type="number" class="form-input w-28" name="discount_value" x-model.number="discount_value" min="0" step="any" placeholder="0" @input="if(discount_type==='percent' && discount_value>100) discount_value=100; calculate()" />
                            </div>
                        </div>
                        ...
                        <div class="rounded-lg border border-gray-200 dark:border-[#1b2e4b] overflow-hidden">
                            ...
                            <div class="p-3">
                                <input type="number" class="form-input w-full" name="advance_received" value="{{ old('advance_received', 0) }}" min="0" step="any" placeholder="0" />
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('proformaForm', () => ({
                items: [{ product_id: '', description: '', hsn_code: '', quantity: 1, unit: 'pcs', rate: 0, discount_percent: 0, tax_percent: 0, line_total: 0 }],
