<x-layout.admin title="Edit Quotation">
    <div x-data="quotationEditForm()">
        <x-admin.breadcrumb :items="[['label'=>'Quotations','url'=>route('admin.quotations.index')],['label'=>'Edit Quotation']]" />

        <div class="flex items-center justify-between mb-6">
            <div>
                <h5 class="text-xl font-bold dark:text-white-light">Edit Quotation</h5>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    Editing <span class="font-semibold text-primary">{{ $quotation->quotation_number }}</span>
                </p>
            </div>
            <a href="{{ route('admin.quotations.index') }}" class="btn btn-outline-primary gap-2">
        ...
        <form action="{{ route('admin.quotations.update', $quotation->id) }}" method="POST">
            ...
            <div class="panel mb-5 !p-0">
                <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-primary/15 rounded-t-xl overflow-hidden">
                    ...
                    <div>
                        <h6 class="font-semibold text-primary text-sm">Quotation Details</h6>
                ...
                <div class="p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="lg:col-span-1">
                            ...
                            <x-admin.searchable-select name="customer_id" :options="$customers" :selected="$quotation->customer_id" placeholder="-- Select Customer --" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">Quotation Date <span class="text-danger">*</span></label>
                            <input name="quotation_date" type="date" class="form-input"
                                value="{{ old('quotation_date', $quotation->quotation_date ? \Carbon\Carbon::parse($quotation->quotation_date)->format('Y-m-d') : '') }}" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">Valid Until</label>
                        ...
                        <div>
                            ...
                            <select name="status" class="form-select">
                                @foreach(['draft'=>'Draft','sent'=>'Sent','accepted'=>'Accepted','rejected'=>'Rejected','expired'=>'Expired'] as $val=>$label)
                                    <option value="{{ $val }}" {{ old('status', $quotation->status) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="divide-y divide-gray-100 dark:divide-[#1b2e4b]">
                    <template x-for="(item, index) in items" :key="index">
                        <div class="px-5 py-4 group">
                            {{-- Row 1: # badge | Product | Description | HSN/SAC | Delete --}}
                            ...
                            {{-- Row 2: Qty | Unit | Rate | Disc% | Tax% | Total --}}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                ...
                <div class="panel !p-0">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            ...
                            <textarea name="terms" class="form-input resize-none" rows="5" placeholder="Payment terms, delivery conditions...">{{ old('terms', $quotation->terms) }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea name="notes" class="form-input resize-none" rows="4" placeholder="Notes visible only to your team...">{{ old('notes', $quotation->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel !p-0 flex flex-col">
                    ...
                    <div class="px-5 pb-5 pt-2 flex flex-col sm:flex-row gap-2 justify-end border-t border-gray-100 dark:border-[#1b2e4b] mt-auto">
                        <a href="{{ route('admin.quotations.index') }}" class="btn btn-outline-secondary gap-2">
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('quotationEditForm', () => ({
                items: @json($quotationItems),
                discount_type: @json(old('discount_type', $quotation->discount_type ?? 'percent')),
                discount_value: {{ old('discount_value', $quotation->discount_value ?? 0) }},
                tax_percent: {{ old('tax_percent', $quotation->tax_percent ?? 0) }},
                ...
                init() {
                    ...
                    // Recalculate every line fresh, then totals once
