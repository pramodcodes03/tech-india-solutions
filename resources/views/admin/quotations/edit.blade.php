<x-layout.admin>
    <div x-data="quotationEditForm()">
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Edit Quotation</h5>
        ...
        <form action="{{ route('admin.quotations.update', $quotation->id) }}" method="POST">
            ...
            <div class="panel mb-5">
                <h6 class="text-base font-semibold mb-4">Quotation Details</h6>
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                    ...
                    <div>
                        ...
                        <input id="quotation_date" name="quotation_date" type="date" class="form-input" value="{{ old('quotation_date', $quotation->quotation_date) }}" required />
                    </div>
                    <div>
                        <label for="valid_until">Valid Until</label>
                        <input id="valid_until" name="valid_until" type="date" class="form-input" value="{{ old('valid_until', $quotation->valid_until) }}" />
                    </div>
                    <div>
            ...
            <div class="panel mb-5">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <textarea id="terms" name="terms" class="form-input" rows="4" placeholder="Enter terms and conditions...">{{ old('terms', $quotation->terms) }}</textarea>
                    </div>
                    <div>
                        <label for="notes">Notes</label>
                        <textarea id="notes" name="notes" class="form-input" rows="4" placeholder="Internal notes...">{{ old('notes', $quotation->notes) }}</textarea>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('quotationEditForm', () => ({
                items: @json($quotation->items->map(fn($i) => [
                ...
                discount_type: '{{ old('discount_type', $quotation->discount_type ?? 'percent') }}',
