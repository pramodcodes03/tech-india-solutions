<x-layout.admin title="Users">
    <div x-data="adminUserList">
        <x-admin.breadcrumb :items="[['label' => 'Admin Users']]" />

        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Admin Users</h5>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <select class="form-select py-2 w-48" x-model="filterRole" @change="fetchData(1)">
                ...
                <button type="button" class="btn btn-outline-danger btn-sm" x-show="searchText || filterRole || filterStatus" @click="clearFilters()">Clear</button>
                <a href="{{ route('admin.admin-users.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2" x-text="item.phone || '-'"></td>
                                <td class="px-4 py-2">
                                    <span class="badge bg-primary" x-text="item.roles && item.roles.length ? item.roles[0].name : '-'"></span>
                                </td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2" x-text="item.last_login_at || '-'"></td>
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/admin-users') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary p-1.5" data-tippy-content="Edit"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                        ...
                        <tr x-show="items.length === 0" x-cloak>
                            <x-admin.empty-state
                                icon="users"
                                title="No users yet"
                                description="Add team members to give them access to the system."
                                action-url="{{ route('admin.admin-users.create') }}"
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('adminUserList', () => ({
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.admin-users.index') }}?page=${page}`;
                    if (this.searchText) url += `&search=${encodeURIComponent(this.searchText)}`;
                    if (this.filterRole) url += `&role_id=${this.filterRole}`;
                ...
                toggleStatus(id) {
                    fetch(`{{ url('admin/admin-users') }}/${id}/toggle-status`, {
                        method: 'PATCH',
                ...
                deleteItem(id) {
                    ...
                    swalWithButtons.fire({ title: 'Are you sure?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/admin-users') }}/${id}`, {
                                method: 'DELETE',
                                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                            })
                            .then(res => res.json())
