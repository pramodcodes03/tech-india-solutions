    ...
    <form method="POST" action="{{ route('admin.assets.bulk.apply') }}" id="bulkForm">
        ...
        <div class="panel p-4 mb-4 sticky top-0 z-10">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                ...
                <div class="md:col-span-2 hidden" data-field="status"><label class="text-xs text-gray-500">Status</label><select name="status" class="form-select"><option value="">—</option>@foreach(['draft','in_storage','assigned','in_maintenance','retired','disposed'] as $s)<option value="{{ $s }}">{{ ucfirst(str_replace('_',' ',$s)) }}</option>@endforeach</select></div>
