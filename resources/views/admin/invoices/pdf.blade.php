...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .totals-table .amount-due td { background-color: #1a1a2e; color: #fff; font-size: 14px; padding: 10px; }
        .payment-info { margin-bottom: 25px; }
        .payment-info table { width: 100%; border-collapse: collapse; }
        .payment-info th { background-color: #f0f0f0; padding: 8px; text-align: left; font-size: 10px; font-weight: bold; text-transform: uppercase; border-bottom: 1px solid #ddd; }
        .payment-info td { padding: 6px 8px; font-size: 11px; border-bottom: 1px solid #eee; }
    ...
    <div class="container">
        ...
        <div class="client-section">
            <div class="client-box">
                ...
                <div class="client-details">
                    @if($invoice->customer->address ?? false){{ $invoice->customer->address }}<br>@endif
                    @if($invoice->customer->city ?? false){{ $invoice->customer->city->name ?? '' }}@endif
                    @if($invoice->customer->state ?? false), {{ $invoice->customer->state }}@endif
                    @if($invoice->customer->pincode ?? false) - {{ $invoice->customer->pincode }}@endif
                    @if($invoice->customer->phone ?? false)<br>Phone: {{ $invoice->customer->phone }}@endif
                    @if($invoice->customer->email ?? false)<br>Email: {{ $invoice->customer->email }}@endif
                    @if($invoice->customer->gstin ?? false)<br>GSTIN: {{ $invoice->customer->gstin }}@endif
                </div>
            </div>
        </div>
                ...
                <table class="totals-table">
                    ...
                    @if($invoice->discount_value > 0)
                        <tr>
                            ...
                            <td>- {{ number_format($invoice->discount_type === 'percent' ? ($invoice->subtotal * $invoice->discount_value / 100) : $invoice->discount_value, 2) }}</td>
                        </tr>
                    @endif
                    @if($invoice->tax_amount > 0)
                        <tr>
                            <td>Tax ({{ $invoice->tax_percent ?? 0 }}%)</td>
        ...
        @if($invoice->payments && $invoice->payments->count() > 0)
            <div class="payment-info">
                <div class="terms-title">Payment History</div>
                <table>
                    <thead>
                        <tr>
                            <th>Payment #</th>
