<x-layout.admin title="Purchase Orders">
    <div x-data="purchaseOrderList">
        <x-admin.breadcrumb :items="[['label' => 'Purchase Orders']]" />

        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Purchase Orders</h5>
            <div class="flex items-center gap-3 flex-wrap">
                <div class="relative">
                    <input type="text" placeholder="Search PO# / Vendor..."
                ...
                <input type="date" class="form-input py-2 w-40" x-model="filterDateFrom" @change="fetchData(1)" placeholder="From Date" />
                <input type="date" class="form-input py-2 w-40" x-model="filterDateTo" @change="fetchData(1)" placeholder="To Date" />
                ...
                <a href="{{ route('admin.purchase-orders.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2 font-semibold" x-text="item.po_number"></td>
                                <td class="px-4 py-2" x-text="item.vendor ? item.vendor.name : '-'"></td>
                                <td class="px-4 py-2" x-text="formatDate(item.po_date)"></td>
                                <td class="px-4 py-2" x-text="formatDate(item.expected_date)"></td>
                                <td class="px-4 py-2">
                                    <span class="badge"
                                        :class="{
                                            'bg-dark': item.status === 'draft',
                                            'bg-info': item.status === 'sent',
                                            'bg-warning': item.status === 'partial',
                                            'bg-success': item.status === 'received',
                                            'bg-danger': item.status === 'cancelled'
                                        }"
                                        x-text="item.status.charAt(0).toUpperCase() + item.status.slice(1)">
                                    </span>
                                </td>
                                <td class="px-4 py-2 text-right font-semibold" x-text="formatCurrency(item.grand_total)"></td>
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/purchase-orders') }}/${item.id}`" class="btn btn-sm btn-outline-info p-1.5" data-tippy-content="View Details"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        <a :href="`{{ url('admin/purchase-orders') }}/${item.id}/edit`"
                                            class="btn btn-sm btn-outline-primary"
                                            :class="(item.status === 'received' || item.status === 'cancelled') && 'opacity-40 pointer-events-none'"
                                            :tabindex="(item.status === 'received' || item.status === 'cancelled') ? -1 : 0">Edit</a>
                        ...
                        <tr x-show="items.length === 0">
                            <x-admin.empty-state
                                ...
                                description="Create your first purchase order to start sourcing from vendors."
                                action-url="{{ route('admin.purchase-orders.create') }}"
                                action-label="Create PO"
                                :colspan="8"
                            />
                        </tr>
                    </tbody>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('purchaseOrderList', () => ({
                items: @json($purchaseOrders->items()),
                pagination: {
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.purchase-orders.index') }}?page=${page}`;
                ...
                deleteItem(id) {
                    ...
                    swalWithButtons.fire({ title: 'Are you sure?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/purchase-orders') }}/${id}`, {
                                method: 'DELETE',
