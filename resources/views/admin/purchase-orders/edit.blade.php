<x-layout.admin title="Edit PO">
    <div x-data="purchaseOrderEditForm()">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h5 class="text-xl font-bold dark:text-white-light">Edit Purchase Order</h5>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Update the details of this purchase order</p>
            </div>
            <a href="{{ route('admin.purchase-orders.index') }}" class="btn btn-outline-primary gap-2">
            ...
            <div class="panel mb-5 !p-0">
                <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-primary/15 rounded-t-xl overflow-hidden">
                    <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/20 shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                    </div>
                    <div>
                        <h6 class="font-semibold text-primary text-sm">Purchase Order Details</h6>
                        <p class="text-xs text-gray-400 mt-0.5">Vendor, dates and status</p>
                    </div>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="lg:col-span-1">
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">Vendor <span class="text-danger">*</span></label>
                            <x-admin.searchable-select name="vendor_id" :options="$vendors" :selected="$purchaseOrder->vendor_id" placeholder="-- Select Vendor --" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">PO Date <span class="text-danger">*</span></label>
                            <input name="po_date" type="date" class="form-input" value="{{ old('po_date', $purchaseOrder->po_date) }}" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">Expected Date</label>
                            <input name="expected_date" type="date" class="form-input" value="{{ old('expected_date', $purchaseOrder->expected_date) }}" />
                        </div>
                        <div>
                            ...
                            <select name="status" class="form-select">
                                ...
                                <option value="partial" {{ old('status', $purchaseOrder->status) === 'partial' ? 'selected' : '' }}>Partial</option>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                ...
                <div class="panel !p-0">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            ...
                            <textarea name="terms" class="form-input resize-none" rows="5" placeholder="Payment terms, delivery conditions...">{{ old('terms', $purchaseOrder->terms) }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea name="notes" class="form-input resize-none" rows="4" placeholder="Notes visible only to your team...">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel !p-0 flex flex-col">
                    <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-success/10 via-success/5 to-transparent border-b border-success/15">
                        ...
                        <div>
                            <h6 class="font-semibold text-success text-sm">Order Summary</h6>
                    ...
                    <div class="px-5 pb-5 pt-2 flex flex-col sm:flex-row gap-2 justify-end border-t border-gray-100 dark:border-[#1b2e4b] mt-auto">
                        <a href="{{ route('admin.purchase-orders.index') }}" class="btn btn-outline-secondary gap-2">
                        ...
                        <button type="submit" name="action" value="draft" class="btn btn-dark gap-2">
                        ...
                        <button type="submit" name="action" value="send" class="btn btn-primary gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                            Update & Send
                        </button>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('purchaseOrderEditForm', () => ({
                items: @json($purchaseOrder->items->map(fn($i) => [
                ...
                discount_type: '{{ old('discount_type', $purchaseOrder->discount_type ?? 'percent') }}',
                discount_value: {{ old('discount_value', $purchaseOrder->discount_value ?? 0) }},
                tax_percent: {{ old('tax_percent', $purchaseOrder->tax_percent ?? 0) }},
                ...
                selectProduct(index) {
                    ...
                    if (product) {
                        ...
                        item.rate = parseFloat(product.purchase_price || product.cost_price || product.price || 0);
