...
/* ── Module "Select All" checkbox (larger) ── */
...
.dark .save-bar { background: rgba(14,23,38,0.95); border-top-color: #1b2e4b; }
.save-bar.visible { transform: translateY(0); }

/* Filter input */
.filter-input {
    border: 1.5px solid #e5e7eb; border-radius: 8px;
    padding: 8px 14px 8px 36px; font-size: 13px; outline: none;
    transition: border-color 0.2s; background: #fff; color: #374151;
}
...
.global-all {
    ...
    border: 1.5px solid var(--v); color: var(--v);
    font-size: 13px; font-weight: 600; transition: all 0.2s;
    background: transparent;
}
.global-all:hover { background: var(--v); color: #fff; }
.global-all.active { background: var(--v); color: #fff; }
</style>

<div x-data="permGrid()" x-init="init()" @keydown.window.escape="dirty=false">
    <x-admin.breadcrumb :items="[['label'=>'Roles','url'=>route('admin.roles.index')],['label'=>'Edit Role']]" />

    <div class="flex items-center justify-between mb-6">
        <div>
            <h5 class="text-xl font-bold dark:text-white">Edit Role &mdash; <span style="color:var(--v)">{{ $role->name }}</span></h5>
            <p class="text-sm text-gray-500 mt-0.5"><span x-text="checkedCount">{{ count($rolePermissions) }}</span> permissions active across all modules</p>
        </div>
    ...
    <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" id="roleForm" @change="dirty=true">
        @csrf @method('PUT')
            ...
            <div class="flex flex-wrap items-end gap-6">
                ...
                <div class="flex gap-4">
                    <div class="text-center px-5 py-3 rounded-xl" style="background:var(--vl)">
                        <div class="text-2xl font-black" style="color:var(--v)" x-text="checkedCount">{{ count($rolePermissions) }}</div>
                        <div class="text-xs text-gray-500 font-medium">Permissions</div>
                    </div>
                    <div class="text-center px-5 py-3 rounded-xl" style="background:rgba(16,185,129,0.08)">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">Click a row to expand &amp; manage actions</p>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($modules as $module)
                                @php
                                    ...
                                    $totalForMod = count($modulePerms);
                                    $checkedForMod = count(array_filter($modulePerms, fn($p) => in_array($p, old('permissions', $rolePermissions))));
                                    $moduleSlug = str_replace([' ', '_'], '-', $module);
                                @endphp
                                <tr x-show="!search || '{{ strtolower(str_replace(['_','-'], ' ', $module)) }}'.includes(search.toLowerCase())"
                                    class="module-row" id="row-{{ $moduleSlug }}">
                                        ...
                                        <div class="mod-name">
                                            <div class="mod-icon">
                                                @php
                                                    ...
                                                    $iconPath = $icons[$module] ?? '<path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>';
                                                @endphp
                                            ...
                                            <div>
                                                ...
                                                <div class="prog-bar-wrap">
                                                    <div class="prog-bar" id="prog-{{ $moduleSlug }}"
                                                        style="width:{{ $totalForMod > 0 ? round($checkedForMod/$totalForMod*100) : 0 }}%"></div>
                                                </div>
                                                @endif
                                            </div>
                                        </div>
        ...
        <div class="save-bar" :class="{ visible: dirty }">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-semibold text-gray-700 dark:text-white">Unsaved changes — <span x-text="checkedCount"></span> permissions selected</span>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('admin.roles.index') }}" class="btn btn-outline-secondary" @click="dirty=false">Discard</a>
        ...
        {{-- Static save buttons (always visible) --}}
        <div class="flex justify-end gap-3 mt-5" style="margin-bottom:10px">
...
function permGrid() {
    return {
        checkedCount: {{ count($rolePermissions) }},
        allChecked: false,
        search: '',
        dirty: false,

        init() {
            this.$nextTick(() => this.updateAll());
        },
        ...
        toggleAll(val) {
            ...
            @foreach($modules as $module)
                @php $slug = str_replace([' ','_'], '-', $module); @endphp
                this.updateProgress('{{ $slug }}');
            @endforeach
            this.updateAll();
            this.dirty = true;
        },
        ...
        updateProgress(module) {
            ...
            const pct  = Math.round(Array.from(cbs).filter(c => c.checked).length / cbs.length * 100);
            bar.style.width = pct + '%';
        }
    }
}
