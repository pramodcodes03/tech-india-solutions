    ...
    <div x-data="productList">
        ...
        <div class="flex items-center justify-between gap-4 mb-5">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <div class="relative">
                    <input type="text" placeholder="Search by name/code..."
                ...
                <button type="button" class="btn btn-outline-danger btn-sm" x-show="searchText || filterCategory || filterStatus" @click="clearFilters()">Clear</button>
                @can('products.create')
                <a href="{{ route('admin.products.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2" x-text="item.category ? item.category.name : '-'"></td>
                                <td class="px-4 py-2" x-text="item.unit || '-'"></td>
                                <td class="px-4 py-2" x-text="formatCurrency(item.purchase_price)"></td>
                                <td class="px-4 py-2" x-text="formatCurrency(item.selling_price)"></td>
                                <td class="px-4 py-2" x-text="item.mrp ? formatCurrency(item.mrp) : '-'"></td>
                                <td class="px-4 py-2" x-text="item.tax_percent != null ? item.tax_percent + '%' : '-'"></td>
                                <td class="px-4 py-2" x-text="item.current_stock ?? 0"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/products') }}/${item.id}`" class="btn btn-sm btn-outline-info p-1.5" data-tippy-content="View Details"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        @can('products.edit')<a :href="`{{ url('admin/products') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary p-1.5" data-tippy-content="Edit"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>@endcan
                                        @can('products.delete')<button type="button" class="btn btn-sm btn-outline-danger p-1.5" @click="deleteItem(item.id)" data-tippy-content="Delete"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>@endcan
                                    </div>
                                </td>
                            </tr>
                        </template>
                        <tr x-show="items.length === 0">
                            <x-admin.empty-state
                                ...
                                description="Add your first product to start managing your catalog and inventory."
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('productList', () => ({
                ...
                toggleStatus(id) {
                    fetch(`{{ url('admin/products') }}/${id}/toggle-status`, {
                        method: 'PATCH',
