<x-layout.admin title="Edit Order">
    <div x-data="salesOrderEditForm()">
        <x-admin.breadcrumb :items="[['label'=>'Sales Orders','url'=>route('admin.sales-orders.index')],['label'=>'Edit Order']]" />

        <div class="flex items-center justify-between mb-6">
            <div>
                <h5 class="text-xl font-bold dark:text-white-light">Edit Sales Order</h5>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Update the details for this sales order</p>
            </div>
            <a href="{{ route('admin.sales-orders.index') }}" class="btn btn-outline-primary gap-2">
        ...
        <form action="{{ route('admin.sales-orders.update', $salesOrder->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="panel mb-5 !p-0">
                <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-primary/15 rounded-t-xl overflow-hidden">
                    ...
                    <div>
                        <h6 class="font-semibold text-primary text-sm">Order Details</h6>
                        <p class="text-xs text-gray-400 mt-0.5">Customer, date and status</p>
                    </div>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="lg:col-span-1">
                            ...
                            <x-admin.searchable-select name="customer_id" :options="$customers" :selected="$salesOrder->customer_id" placeholder="-- Select Customer --" required />
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">Order Date <span class="text-danger">*</span></label>
                            <input name="order_date" type="date" class="form-input" value="{{ old('order_date', $salesOrder->order_date) }}" required />
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5 block">From Quotation</label>
                            <input type="text" class="form-input bg-gray-100 dark:bg-gray-800" value="{{ $salesOrder->quotation->quotation_number ?? '-' }}" readonly />
                            <input type="hidden" name="quotation_id" value="{{ $salesOrder->quotation_id }}" />
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                ...
                <div class="panel !p-0">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            ...
                            <textarea name="terms" class="form-input resize-none" rows="5" placeholder="Payment terms, delivery conditions...">{{ old('terms', $salesOrder->terms) }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea name="notes" class="form-input resize-none" rows="4" placeholder="Notes visible only to your team...">{{ old('notes', $salesOrder->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel !p-0 flex flex-col">
                    ...
                    <div class="px-5 pb-5 pt-2 flex flex-col sm:flex-row gap-2 justify-end border-t border-gray-100 dark:border-[#1b2e4b] mt-auto">
                        <a href="{{ route('admin.sales-orders.index') }}" class="btn btn-outline-secondary gap-2">
