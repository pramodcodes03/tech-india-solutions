<x-layout.admin title="Add Customer">
    <div x-data="customerLocation({{ Js::from(['state' => old('state'), 'city' => old('city'), 'cities' => $cities->map(fn($c) => ['id' => $c->id, 'name' => $c->name, 'state' => $c->state])->values()]) }})">
            ...
            <form action="{{ route('admin.customers.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="credit_limit" name="credit_limit" type="number" step="0.01" class="form-input" value="{{ old('credit_limit') }}" />
                    </div>
                    <div class="md:col-span-2">
                        <label for="billing_address">Billing Address</label>
                        <textarea id="billing_address" name="billing_address" class="form-input" rows="2">{{ old('billing_address') }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label for="shipping_address">Shipping Address</label>
                        <textarea id="shipping_address" name="shipping_address" class="form-input" rows="2">{{ old('shipping_address') }}</textarea>
                    </div>
                        ...
                        <div class="relative" @click.outside="cityOpen = false">
                            ...
                            <div x-show="cityOpen" x-cloak x-transition class="absolute z-50 mt-1 w-full bg-white dark:bg-[#1b2e4b] border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-64 overflow-hidden flex flex-col">
                                <div class="p-2 border-b border-gray-200 dark:border-gray-700">
                                    <input type="text" x-model="citySearch" x-ref="citySearchBox" @keydown.escape="cityOpen = false"
