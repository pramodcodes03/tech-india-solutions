<x-layout.admin title="Customers">
    <div x-data="customerList">
        ...
        <div class="flex items-center justify-between gap-4 mb-5">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <select class="form-select py-2 w-40" x-model="filterState" @change="fetchData(1)">
                ...
                <button type="button" class="btn btn-outline-danger btn-sm" x-show="searchText || filterState || filterCity || filterStatus" @click="clearFilters()">Clear</button>
                <a href="{{ route('admin.customers.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2" x-text="item.state || '-'"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-1">
                                        <a :href="`{{ url('admin/customers') }}/${item.id}`" class="btn btn-sm btn-outline-info p-1.5" data-tippy-content="View Details">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>
                                        <a :href="`{{ url('admin/customers') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary p-1.5" data-tippy-content="Edit">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-danger p-1.5" @click="deleteItem(item.id)" data-tippy-content="Delete">
                        ...
                        <tr x-show="items.length === 0">
                            <x-admin.empty-state
                                ...
                                description="Start by adding your first customer to manage their orders and history."
                                action-url="{{ route('admin.customers.create') }}"
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('customerList', () => ({
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.customers.index') }}?page=${page}`;
